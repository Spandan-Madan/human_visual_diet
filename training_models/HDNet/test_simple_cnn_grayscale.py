...
from core.dataset_grayscale import COCODatasetWithID
...
NUM_CLASSES = 15
import timm
def test(cfg, model, annotations_file, imagedir, outdir, outname="test", epoch=None, record_individual_scores=False, print_batch_metrics=False):
    """
    Arguments:
...
#     assert not missing_keys, "Checkpoint is missing keys required to initialize the model: {}".format(missing_keys)
    ...
    print('args is')
    print(args)
#     print(cfg.test_annotations, cfg.test_imagedir, args.outdir, args.outname, args.record_individual_scores , args.print_batch_metrics)
    test(cfg, model, cfg.test_annotations, cfg.test_imagedir, outdir=args.outdir, outname=args.outname, epoch=None, record_individual_scores=args.record_individual_scores , print_batch_metrics=args.print_batch_metrics)
