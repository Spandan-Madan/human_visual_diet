...
# with open('/n/pfister_lab2/Lab/spandan/contextual_domain_adaptation/args_hotswapped.p','wb') as File:
...
cfg.vit_baseline = args.vit_baseline
...
print('Pretrained is set to:')
print(cfg.pretrained)
print('blur target')
print(cfg.blur_target)
print('blur context')
print(cfg.blur_context)
...
dataset = COCODataset(cfg.annotations, cfg.imagedir, (224,224), material_fols, normalize_means=[0.485, 0.456, 0.406], normalize_stds=[0.229, 0.224, 0.225], geometric_transforms = cfg.geometric_transforms, color_transforms = cfg.color_transforms,complex_transforms=cfg.complex_transforms)
...
#         labels_cpu = torch.hstack((info_1[3], info_2[3]))
    ...
    if cfg.test_annotations is not None and cfg.test_imagedir is not None and epoch % args.test_frequency == 0:
        ...
        test_accuracy = test(model, cfg, cfg.test_annotations, cfg.test_imagedir, outdir=args.outdir, epoch=epoch)
