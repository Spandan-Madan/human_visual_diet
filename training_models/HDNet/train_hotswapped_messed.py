...
from core.dataset_single_domain import COCODataset
...
for epoch in range(start_epoch, args.epochs + 1):
    ...
    for i, (context_images, target_images, bbox, labels_cpu) in enumerate(dataloader):
        if i%100==0:
            print("batch num:%s"%i)
            continue
        # info_1, info_2 = data
...
#         labels_cpu = torch.hstack((info_1[3], info_2[3]))
        ...
        context_images_all = context_images
        target_images_all = target_images
        bbox_all = bbox
        labels_all = labels
        labels_all_cpu = labels_cpu
