...
criterion_4 = SupConLoss(temperature=0.1)
...
#         labels_cpu = torch.hstack((info_1[3], info_2[3]))
        ...
        f1_, f2_ = torch.split(output_uncertainty_branch, [args.batch_size, args.batch_size], dim=0)
        features_ = torch.cat([f1_.unsqueeze(1), f2_.unsqueeze(1)], dim=1)
        normalized_features_ = F.normalize(features_, dim = 2)
        ...
        if args.hard_mining == 'both':
            ...
            loss_contrastive_ = criterion_4(normalized_features_, labels)
            overall_main_loss = loss_main_branch + 0.5*loss_contrastive + 0.5*loss_contrastive_
