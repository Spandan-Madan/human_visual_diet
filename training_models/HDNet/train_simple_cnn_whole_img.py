...
parser.add_argument("--two_d_transforms",action='store_true')
...
# print('saved, exiting...')
# sys.exit()
dataset = COCODataset(cfg.annotations, cfg.imagedir, image_size =(224,224), normalize_means=[0.485, 0.456, 0.406], normalize_stds=[0.229, 0.224, 0.225],two_d_transforms=args.two_d_transforms)
...
with warnings.catch_warnings(): # add_graph method is known to issue a warning
    ...
    writer.add_graph(model, input_to_model=[context_images.to(device)]) # add model graph to tensorboard log
...
#         target_images = target_images.to(device)
        ...
        output_main_branch = model(context_images)
