...
from core.dataset_blurred_context import COCODatasetWithID
...
def test(model, annotations_file, imagedir, outdir, cfg, outname="test", epoch=None, record_individual_scores=False, print_batch_metrics=False):
    """
    Arguments:
    ...
    testset = COCODatasetWithID(annotations_file, imagedir, (224,224), normalize_means=[0.485, 0.456, 0.406], normalize_stds=[0.229, 0.224, 0.225],blur_strength=cfg.blur_strength,context_blur = cfg.context_blur, target_blur = cfg.target_blur)
    ...
    with torch.no_grad():
        for i, (context_images, target_images, bbox, labels_cpu, annotation_ids) in enumerate(tqdm(dataloader, desc="Test Batches", leave=True)):
            ...
            # if cfg.no_target:
            #     target_images = torch.zeros((target_images.shape)).to(device)
            #     print('target set to 0 during test')
            # if cfg.no_context:
            #     context_images = torch.zeros((context_images.shape)).to(device)
            #     print('context set to 0 during test')
...
if __name__ == "__main__":
    ...
    test(model, cfg.test_annotations, cfg.test_imagedir, args.outdir, cfg, outname=args.outname, record_individual_scores=args.record_individual_scores , print_batch_metrics=args.print_batch_metrics)
